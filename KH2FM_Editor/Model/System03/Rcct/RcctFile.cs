...
{
    public class RcctFile : BarSubFile
    {
        ...
        public ObservableCollection<RcctItem> PlayerPrefs { get; set; }

        public RcctFile(String name, List<byte> raw)
        {
            Console.WriteLine("DEBUG >>> Found Magi file: " + name);
            this.Name = name;
            PointerSet = new List<uint>();
            Pointers = new List<uint>();
            PlayerPrefs = new ObservableCollection<RcctItem>();
            processFile(raw);
        }
        ...
        public void processSets(List<byte> raw)
        {
            for (int i = 0; i < PointerSet.Count; i++)
            {
                ...
                PlayerPrefs.Add(new RcctItem(raw.GetRange((int)PointerSet[i], RcctItem.entrySize)));
            }
        ...
        public List<byte> getAsByteList()
        {
            ...
            // Settings
            foreach (RcctItem setting in PlayerPrefs)
            {
